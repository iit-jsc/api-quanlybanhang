...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    const requiredRoleCodes = this.reflector.get<string[]>(
      'roles',
      context.getHandler()
    )
    ...
    const userRoleCodes =
      request.permissions?.reduce((acc, permission) => {
    ...
    if (
      requiredRoleCodes.includes(SPECIAL_ROLE.STORE_OWNER) &&
    ...
    if (
      requiredRoleCodes.includes(SPECIAL_ROLE.MANAGER) &&
    ...
    return requiredRoleCodes.some(requiredCode =>
      userRoleCodeSet.has(requiredCode)
    )
  }
}
