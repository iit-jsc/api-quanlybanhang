...
export function mapResponseLogin(data: AnyObject) {
  ...
  const allRoles = account.permissions.flatMap((permission: Prisma.PermissionCreateInput) => permission.roles);

  const uniqueRoles = Array.from(new Set(allRoles.map((role: Prisma.RoleCreateInput) => role.code))).map((code) =>
